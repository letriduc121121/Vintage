...
router.post("/",async (req,res)=>{
    ...
    try{
        ...
        if(cart){
            ...
            if(productIndex>-1){  // Sản phẩm đã tồn tại trong giỏ hàng, cập nhật số lượng
            ...
            //tong hoa don
            cart.totalPrice=cart.products.reduce((Tongtien,item)=>Tongtien+item.price*item.quantity,0);//acc (accumulator): Tổng giá trị giỏ hàng, ban đầu là 0.
            await cart.save();
    ...
    try{
        ...
        if(guestCart){
            ...
            if(userCart){
                ...
                res.status(200).json.apply(userCart);
            }
            else{
